...
1. **Given** a valid credit card statement PDF, **When** the user runs the application with that file path, **Then** a formatted list of transactions is printed to stdout, each line showing date, description, and amount.
...
- **FR-004**: The application MUST print the list of extracted transactions to standard output as a fixed-width table with aligned columns, a header row, and a separator line beneath the header.
...
- **FR-009**: The application MUST display column labels in the selected output language. For English: "Date", "Description", "Amount". For Brazilian Portuguese: "Data", "Descrição", "Valor".
...
- **Transaction**: A single financial event on the statement. Key attributes: date, description, amount (positive for purchases/charges, negative for payments/credits).
