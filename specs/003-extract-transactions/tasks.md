...
- [ ] T006b [P] Write `tests/unit/transaction_extractor/test_locale.py` for `LOCALE_EN` only — correct date format (`%Y-%m-%d`), empty currency prefix, column labels "Date"/"Description"/"Amount", amount formatting for a known positive and negative Decimal; `LOCALE_PT_BR` tests added in US2 (T015)
- [ ] T006c Implement `src/credit_card_statement_extractor/transaction_extractor/_locale.py` — `LocaleConfig` frozen dataclass with fields: `code`, `date_format`, `decimal_separator`, `thousands_separator`, `currency_prefix`, `col_date`, `col_description`, `col_amount`; export `LOCALE_EN` constant; `LOCALE_PT_BR` added in T017 — T006b tests must pass
- [ ] T007 [P] Write `tests/unit/transaction_extractor/test_formatter.py` — Formatter US1 scenarios (English locale using `LOCALE_EN` from `_locale.py`): header row contains "Date" / "Description" / "Amount", separator line length matches header, each transaction row contains formatted date (YYYY-MM-DD), description, and amount, negative amounts prefixed with `-`, positive amounts prefixed with `+`, column widths accommodate longest value; requires T006c complete
...
- [ ] T017 Update `src/credit_card_statement_extractor/transaction_extractor/_locale.py` — add `LOCALE_PT_BR` constant (`date_format="%d/%m/%Y"`, `decimal_separator=","`, `thousands_separator="."`, `currency_prefix="R$ "`, `col_date="Data"`, `col_description="Descrição"`, `col_amount="Valor"`) — T015 tests must pass
